...
void initWiFi()
{
  ...
  /*
  //-----GET USER-----
  server.on("/getUser", HTTP_GET, [](){
 if(!server.hasArg("userId")) return server.send(500, "text/plain", F("BAD user id!!!"));
  byte id =String(server.arg("userId")).toInt(); 
   if (id>userCount-1) return server.send(500, "text/plain", F("BAD user id<>!!! "));
  String json="{"; 
  json+="\"userId\": \""+String(id)+"\",";
  json+="\"userName\": \""+ipMacPrt[id].userName+"\",";
  json+="\"userTel\": \""+ipMacPrt[id].tel+"\",";
...
server.send(200, "text/plain", F("Saved!"));
  ...
  server.on("/", [](){
    if(!server.authenticate(www_username, www_password))
      ...
      return server.requestAuthentication(DIGEST_AUTH, www_realm, authFailResponse);
    server.send(200, "text/plain", F("Login OK"));
  });
// -----------call GSM TEL. number--------
  server.on("/dial",[](){
 
  if(!server.hasArg("tel")) return server.send(500, "text/plain", "BAD TEL. number!!!");
  String tel = "+38";
  tel+=server.arg("tel");
  msgIn(F("GET TEL:"));msg(tel);
  return server.send(200,"text/plain",  isHangUpCallNumber(tel)?"DODZVONYLYS!":"OSHIBKA!");
  });
  */
  //==========call GSM TEL. number=============
  // -----------call GSM TEL. number--------
  ...
  if(!server.hasArg("comm")) return server.send(500, "text/plain", "BAD TEL. number!!!");
  String log ="["+String(millis())+"]: ";
  log+=sendAtCommand(server.arg("comm"));

   
  return server.send(200,"text/plain",  log);
  });
//===============
*/
  server.begin();
