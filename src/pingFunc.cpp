...
ipMac1[iMac].mac=mac;
ipMac1[iMac].tel=tel;
msgIn(F("Create new user: "));msgIn(ipMac1[iMac].userName);
msgIn(" MAC: "); msgIn(ipMac1[iMac].mac);
...
pinger.OnEnd([](const PingerResponse& response)
  {    
  msgIn(F("IP address: "));msg(String(response.DestIPAddress.toString().c_str()));
       ...
       // ipMacPrt[i_IpMac].mac=String(s);
        ...
        for(byte ii=0;ii<COUNT_USERS;ii++){// 
         ...
         if( usrMac.equalsIgnoreCase(macToString(response.DestMacAddress->addr))){
           msgIn("USER ");msgIn(ipMacPrt[ii].userName); msg(" - ONLINE!");
...
void chekUsersOnLine(){
 static byte iip=ipFrom;
  /*
  msgIn("ping ip:");
  msgIn(String(ipNet[0]));msgIn(".");
  msgIn(String(ipNet[1]));msgIn(".");
  msgIn(String(ipNet[2]));msgIn(".");
  msg(String(iip));
*/
 ...
 msgIn(F("IsUser in home:"));msg(String(usrOnLine?"True":"False"));
